...
> **v0.6.0 已发布** — 体验与健壮性更新。**不想装 Python？** 下载[打包版](#方式一exe-一键运行推荐新手)双击即用，零环境依赖。
...
### v0.6.0（2026-05-26）—— 体验与健壮性更新

- **AI 设定背景板**：可记录 AI 外号/喜好/身份，聊天中自动检测并持久化
- **联网搜索二选一**：大模型 / DuckDuckGo 分开选择
- **大模型联网天气**：AI 自行搜索天气，配合结构化数据注入
- **天气三级容错**：高德 → wttr.in → Open-Meteo，自动重试
- **名字防注入**：所有 prompt 中名字用「」包裹，设置端加验证
- 修复 10+ 项 bug：系统消息打断负面计数、游戏事件泄漏到正常逻辑、主动消息刷屏、datetime 序列化、历史膨胀等
- 天气测试按钮增加 wttr.in 源
- 欢迎语间隔延长（0.5h → 2h），问候去重优化
- 角色预设可删除

详见 [RELEASE_v0.6.0.md](RELEASE_v0.6.0.md)
