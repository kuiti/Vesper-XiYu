...
def _build_depth_hint(user_message: str, emotion: str) -> str:
    """构建深度提示（SillyTavern 方案）：根据上下文动态选择行为强化提示"""
    hints = []
    # 根据用户情绪调整
    if emotion == "negative":
...
def _clean_dsml(text: str) -> str:
    ...
    import re
    # 移除 <DSML tool_calls>...</DSML tool_calls> 整块
...
def _apply_background_update(background_update):
    ...
    if isinstance(background_update, dict):
        ...
        print(f"[AI背景] {action}: {key}={value}")
        return
    # 兼容旧格式：纯文本字符串
    if isinstance(background_update, str):
        bg = background_update.strip()
        if not bg:
            return
        current = get_config("ai_background", "")
        if current:
            try:
                existing = _json.loads(current)
            except _json.JSONDecodeError:
                existing = {"legacy": current}
        else:
            existing = {}
        if bg not in str(existing):
            existing[f"text_{len(existing)}"] = bg
            set_config("ai_background", _json.dumps(existing, ensure_ascii=False))
            print(f"[AI背景] 自动记录: {bg}")
...
def _calc_consecutive_days(msg_count, ws, loop):
    """后台计算连续互动天数（优化：只查最近 60 天，避免全表扫描）+ 触发成就检查"""
    try:
        ...
        with get_conn() as cc:
            cur = cc.execute(
                ...
                "WHERE timestamp >= ? ORDER BY d DESC",
                (cutoff,)
            )
...
def _maybe_surprise(ws, loop):
    ...
    try:
        ...
        if affection >= 30 and hour >= 18 and last_surprise != today and random.random() < 0.1:
            ...
            prompt = f"你是「{ai_name}」，一个AI伙伴。请给用户分享一个小惊喜——可以是一首短诗、一个有趣的冷知识、或一句暖心的话。不超过50字，语气{_tone}。"
...
def _parse_dsml_tool_calls(text: str) -> list:
    """解析 DeepSeek DSML 格式的工具调用文本，返回 [{name, args_dict}, ...]"""
    import re
    results = []
