...
def recall_episode(query: str, limit: int = 3) -> list:
    """按关键词全文检索情景记忆（优先 FTS5，降级到 LIKE）。"""
    try:
        safe_query = _escape_like(query)
        # FTS5 查询：短语匹配，转义内部引号
        fts_query = query.strip().replace('"', '""')
        fts_query = '"' + fts_query + '"' if fts_query else ''
        rows = None
        if fts_query:
            try:
                with get_conn() as conn:
                    ...
                    cursor.execute(
                        """SELECT id, start_time, end_time, emotion_summary, topic_summary,
                           ...
                           FROM episodes
                           WHERE id IN (SELECT rowid FROM episodes_fts WHERE episodes_fts MATCH ?)
...
def get_recent_episodes(limit: int = 5) -> list:
    """获取最近 N 个情景。"""
    try:
        with get_conn() as conn:
            cursor = conn.cursor()
...
def get_episode_timeline(days: int = 7) -> list:
    """按时间线组织最近 N 天的情景。
...
def get_episode_context(query: str = "", limit: int = 3) -> str:
    ...
    """
    if query:
        episodes = recall_episode(query, limit=limit)
    else:
