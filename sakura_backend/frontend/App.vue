<template>
  <div class="app" :data-theme="currentTheme" :style="colorVariables">
    ...
    <div class="app-layout">
      <NavBar :active="activeView" :wsReady="wsReady" :aiName="aiNameLocal" @nav="onNav" @scroll-bottom="onScrollBottom" />
...
export default {
  ...
  data() {
    return {
      ...
      ws: null, wsReady: false, wsReconnectAttempts: 0, _reconnectTimer: null,
  ...
  methods: {
    ...
    connectWebSocket() {
      ...
      this.ws.onmessage = (event) => {
        ...
        if (data.type === 'token') { if (this._systemStreaming) return; this.pendingReply += data.content; this._currentReplyLen = (this._currentReplyLen || 0) + data.content.length; if (!this.schedId) this.schedulePop() }
        ...
        else if (data.type === 'reminder') { this.messages.push({ role: 'assistant', content: '提醒：' + (data.data?.content || data.data?.title || ''), timestamp: new Date().toISOString() }); this.totalMessages++; this.$nextTick(() => this.$refs.chatView?.scrollToBottom()) }
    ...
    _keydownHandler(e) {
      ...
      if (e.key === 'Escape') { if (this.$refs.chatView) this.$refs.chatView.showCtx = false; this.showEmojiPicker = false }
        ...
        else if (e.key === 'n' || e.key === 'N') { e.preventDefault(); if (window.confirm('确定要清空所有聊天记录吗？此操作不可恢复。')) { api.delete('/chat/manage/all').then(() => { this.messages = []; this.totalMessages = 0; this.nextAfterId = null }).catch(() => {}) } }
      }
    },
  }
}
