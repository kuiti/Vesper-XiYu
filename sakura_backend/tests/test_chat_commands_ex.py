...
# ═══════════════════════════════════════════════════════════════
# handle_remind — 补充边界测试
# ═══════════════════════════════════════════════════════════════


class TestHandleRemindEdgeCases:
    @anyio
    async def test_remind_midnight_hour(self, mock_ws):
        """凌晨12点解析。"""
        ...
        with patch("api.chat_commands.add_reminder") as mock_add, \
            ...
            await handle_remind(mock_ws, "/remind 明天12点 午餐")
    ...
    @anyio
    async def test_remind_with_minutes(self, mock_ws):
        """带分钟的时间。"""
        from api.chat_commands import handle_remind

        with patch("api.chat_commands.add_reminder") as mock_add, \
             patch("api.chat_commands.get_reminders", return_value=[]):
            await handle_remind(mock_ws, "/remind 明天9点30分 开会")
    ...
    @anyio
    async def test_remind_sends_reminder_count(self, mock_ws):
        """成功设置后应发送 reminder_count。"""
        from api.chat_commands import handle_remind

        with patch("api.chat_commands.add_reminder"), \
             patch("api.chat_commands.get_reminders", return_value=[
                 {"id": 1, "done": False},
                 {"id": 2, "done": True},
             ]):
            await handle_remind(mock_ws, "/remind 明天9点 测试")

        counts = _get_by_type(mock_ws, "reminder_count")
        assert len(counts) == 1
        assert counts[0]["count"] == 1  # 只算未完成的

    @anyio
    async def test_remind_invalid_hour_25(self, mock_ws):
        """25点应被拒绝。"""
        from api.chat_commands import handle_remind

        await handle_remind(mock_ws, "/remind 25点 测试")
        sent = _get_all_messages(mock_ws)
        assert any("无法识别" in m.get("content", "") for m in sent)