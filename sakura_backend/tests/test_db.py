...
class TestConfigCRUD:
    ...
    def test_set_and_get_dict(self):
        ...
        # 验证返回的是深拷贝（修改不影响缓存）
        result["name"] = "改了"
        assert get_config("character")["name"] == "佐仓"
...
class TestReminders:
    ...
    def test_add_and_get(self):
        from core.db import add_reminder, get_reminders
        now = datetime.now().isoformat()
        add_reminder("记得吃药", now, 4)
        reminders = get_reminders()
        assert len(reminders) >= 1
        # 找到我们添加的那条
        found = [r for r in reminders if r["content"] == "记得吃药"]
        assert len(found) == 1
        assert found[0]["done"] == 0

    def test_done_flag_reflected(self):
        """done 标记正确反映在返回值中。"""
        from core.db import add_reminder, get_reminders
        from core.db import get_conn

        now = datetime.now().isoformat()
        add_reminder("未完成提醒", now, 2)
        add_reminder("已完成提醒", now, 2)

        # 手动标记第二条为已完成
        with get_conn() as conn:
            conn.execute("UPDATE reminders SET done=1 WHERE content='已完成提醒'")

        reminders = get_reminders()
        by_content = {r["content"]: r for r in reminders}
        assert by_content["未完成提醒"]["done"] is False
        assert by_content["已完成提醒"]["done"] is True
