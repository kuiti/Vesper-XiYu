...
def generate_greeting():
    ...
    length_rule = _LENGTH_MAP.get(length_level, "每句话不超过10个字")
    ...
    try:
        ...
        silent_exc("generate_greeting", e)
    # 获取最近几条自己说过的话，避免重复问候
    recent_self = ""
    try:
        ...
        silent_exc("generate_greeting.recent_self", e)
...
def generate_proactive(trigger_type: str, context: dict = None, character_id: int = 0):
    """根据触发类型和上下文生成智能主动问候。
    ...
    Args:
        ...
        character_id: 角色 ID，用于读取对应角色关系数据
    """
    if context is None:
        context = {}
    ...
    # ─── 深夜不打扰（但允许催睡和久别问候）───
    if (hour >= 23 or hour < 7) and trigger_type not in ("late_night", "long_absence"):
        return None
    ...
    try:
        ...
        silent_exc("proactive_greeting.weather", e)
    ...
    if trigger_type == "idle":
        ...
        try:
            ...
            silent_exc("proactive_greeting.comfort_dedup", e)
        ...
        try:
            ...
            silent_exc("proactive_greeting.sustained_low_mood", e)
        empathy_hint = context.get("empathy_style", "")
        if context.get("empathy_avoid"):
            empathy_hint += f"\n注意：{context['empathy_avoid']}"
        ...
        idle_desc = f"现在很晚了（{context.get('hour', datetime.now().hour)}点），「{user_name}」还没睡。"
        trigger_instruction = "温柔催睡，一句话就够，不要啰嗦"
    elif trigger_type == "long_absence":
        days = context.get("days", 3)
        idle_desc = f"「{user_name}」已经{days}天没来找你了。"
        trigger_instruction = "表达想念和关心，确认近况，不要质问"
    ...
    try:
        ...
        silent_exc("proactive_greeting.recent_self", e)
