"""意图识别模块 —— 正则快速路径 + LLM 兜底，支持天气/位置/搜索/知识等意图路由。"""
# version: 6.0.0
import hashlib
import requests
import re
...
from core.weather import get_city_coords, get_openmeteo_weather, wind_direction_name


def build_weather_text(casts: list) -> str:
    """根据时段生成天气播报文本（早/午/晚不同措辞）"""
...
def llm_classify_intent(user_message: str) -> dict:
    """LLM 意图分类（正则未命中时的兜底路径，精简 prompt 降低延迟）
...
用户消息：{user_message[:200]}"""
    ...
    result = call_llm(prompt=prompt, temperature=0, max_tokens=150, timeout=8, json_mode=True)
