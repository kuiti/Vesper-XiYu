...
from core.db import get_config, set_config, get_active_hours, get_proactive_flag, get_proactive_response_rate
import logging
import threading
...
def get_user_active_pattern() -> dict:
    """从 user_activity_stats 学习用户活跃模式，返回 {hour: weight}"""
    try:
        from core.db import get_conn
        with get_conn() as conn:
            cursor = conn.cursor()
            cursor.execute("SELECT hour, message_count FROM user_activity_stats WHERE message_count > 0")
        ...
        pattern = {}
        for r in rows:
            pattern[r["hour"]] = r["message_count"] / total
        return pattern
    except Exception:
        return {}


def get_best_proactive_hour(pattern: dict) -> int | None:
    """根据用户活跃模式，返回最佳主动消息时间（权重最高的小时）"""
    if not pattern:
        return None
    return max(pattern, key=pattern.get)


def is_user_likely_active(pattern: dict, current_hour: int) -> bool:
    """判断用户当前是否 likely 活跃（权重 > 平均值）"""
    if not pattern:
        return True  # 无数据时默认活跃
    avg = sum(pattern.values()) / len(pattern)
    return pattern.get(current_hour, 0) > avg * 0.5
...
def should_proactive_trigger(
    ...
    Args:
        ...
        character_id: 角色 ID（per-character 关系/情绪查询）
    ...
    # ─── 深夜催睡 (23:00-6:00, 30% 概率) ───
    if (23 <= hour or hour < 6) and "late_night" not in session_triggered:
        import random
        if random.random() < 0.3 and willingness >= 0.2:
    ...
    if idle_minutes >= 30 and "idle" not in session_triggered:
        ...
        # 基于用户活跃模式优化：非活跃时段降低触发概率
        pattern = get_user_active_pattern()
        if pattern and not is_user_likely_active(pattern, hour):
            import random
            if random.random() > 0.3:  # 非活跃时段 70% 概率跳过
                return False, "", {}
        return True, "idle", {
...
CARE_CATEGORIES = {
    ...
    "care": {"name": "日常关怀", "max_daily": 3, "style": "温暖关心，不啰嗦"},
}
...
_TRIGGER_TO_CATEGORY = {
    ...
    "late_night": "care", "long_absence": "care",
}
...
def get_care_category(trigger_type: str) -> str:
    """根据触发类型返回关怀类别（emotion/bonding/task/care）"""
...
def should_heartbeat_trigger(idle_minutes: float, session_triggered: set = None, character_id: int = 0) -> tuple:
    """判断是否触发心跳式主动问候（8-22 点，间隔 2h+，per-character）"""
    if session_triggered is None:
        session_triggered = set()
    now = datetime.now()
    hour = now.hour
