# core/goal_tracker.py — 长期目标追踪引擎（per-character）
"""
...
def ingest_keypoints(key_points: list, character_id: int = 0):
    """接收 summary_engine 新产生的 key_points，识别目标/计划类并写入 goal_tracking（per-character）"""
    if not key_points:
        return
...
不是目标/计划的要点不要包含。如果没有目标/计划，输出空数组。"""
    ...
    with get_chat_conn(character_id) as conn:
        ...
        if goals:
            logger.info(f"[GoalTracker] 角色{character_id} 录入 {len(goals)} 个目标: {[g.get('text', '')[:30] for g in goals]}")


def get_pending_goals(limit: int = 3, character_id: int = 0) -> list:
    """获取待确认的目标列表（per-character）"""
    with get_chat_conn(character_id) as conn:
        cursor = conn.cursor()
...
def confirm_goal(pending_id: int, character_id: int = 0) -> bool:
    """确认目标：从暂存移入正式跟踪（per-character）"""
    with get_chat_conn(character_id) as conn:
        cursor = conn.cursor()
...
def reject_goal(pending_id: int, character_id: int = 0) -> bool:
    """拒绝目标（per-character）"""
    with get_chat_conn(character_id) as conn:
...
def get_stale_goals(days_threshold: int = 7, limit: int = 3, character_id: int = 0) -> list:
    """查询状态为 active 且超过 days_threshold 天未提及的目标（per-character）"""
    with get_chat_conn(character_id) as conn:
        cursor = conn.cursor()
        cursor.execute(
...
def update_goal_status(goal_id: int, status: str, character_id: int = 0):
    """手动更新目标状态（per-character）"""
    with get_chat_conn(character_id) as conn:
        conn.cursor().execute(
...
def record_follow_up(goal_id: int, character_id: int = 0):
    """记录一次主动跟进（per-character）"""
    now = datetime.now().isoformat()
    with get_chat_conn(character_id) as conn:
        cursor = conn.cursor()
        cursor.execute(
...
def check_goal_completion(user_message: str, character_id: int = 0) -> list:
    """用 LLM 判断用户是否在对话中暗示完成了某个目标（per-character）"""
    with get_chat_conn(character_id) as conn:
        cursor = conn.cursor()
    ...
    with get_chat_conn(character_id) as conn:
        ...
        for gid in completed:
            try:
                ...
                logger.info(f"[GoalTracker] 角色{character_id} 目标 #{gid} 标记为完成")
        ...
        for gid in abandoned:
            try:
                ...
                logger.info(f"[GoalTracker] 角色{character_id} 目标 #{gid} 标记为放弃")
            except (ValueError, TypeError):
                logger.warning(f"[GoalTracker] 无效目标 ID: {gid}")
...
def get_goal_stats(character_id: int = 0) -> dict:
    """目标统计（per-character）"""
    with get_chat_conn(character_id) as conn:
        cursor = conn.cursor()
