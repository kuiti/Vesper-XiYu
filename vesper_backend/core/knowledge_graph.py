...
def extract_triplets_from_messages(messages: list, llm_call_func=None) -> list:
    """从消息列表中提取实体关系三元组，返回 [{"subject", "predicate", "object"}] 列表。"""
...
def extract_triples_from_text(text: str, source: str = "chat", character_id: int = 0) -> list:
    """从单条文本中提取三元组并自动保存到数据库。
    text 格式建议包含对话双方：'用户: xxx\nAI: xxx'
    """
    if not text or len(text) < 20:
        return []
    # 不依赖 role 过滤，直接对原始文本调用 LLM
    try:
        from core.llm_client import call_llm
        prompt = _EXTRACT_PROMPT.format(content=text[:2000])
        result = call_llm(prompt=prompt, temperature=0, max_tokens=500, timeout=15, json_mode=True)
        if isinstance(result, list):
            ...
            if triplets:
                save_triplets(triplets, source_msg_id=None, character_id=character_id)
            return triplets
    except Exception as e:
        logger.warning(f"[知识图谱] 提取失败: {e}")
    return []
...
def save_triplets(triplets: list, source_msg_id=None, character_id: int = 0):
    """将三元组列表批量保存到知识图谱数据库（per-character）。"""
    for t in triplets:
        try:
...
def get_context_for_message(message: str, character_id: int = 0) -> str:
    """根据用户消息中的实体检索相关三元组，返回可注入 prompt 的上下文文本（per-character）。"""
    try:
        import jieba
    except ImportError:
        jieba = None
    ...
    for entity in entities[:5]:  # 最多查询5个实体
        triplets = query_knowledge_by_entity(entity, character_id=character_id)
