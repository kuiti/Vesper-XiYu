...
def _consolidate_recent_important() -> int:
    ...
    try:
        ...
        for row in rows:
            ...
            # 检查是否命中高权重 mention（用户反复提过的）
            hits_mention = False
            if top_mentions:
                try:
                    import jieba
                    content_phrases = {w for w in jieba.cut(content) if len(w) >= 2}
                    hits_mention = bool(content_phrases & top_mentions)
                except Exception:
                    pass

            # 既不在静态关键词列表，也没命中 mention → 跳过
