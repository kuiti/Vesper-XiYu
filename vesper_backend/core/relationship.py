...
_rel_cache = {}  # {character_id: {"ts": float, "value": tuple}}
_rel_cache_lock = threading.Lock()
_REL_CACHE_TTL = 5.0  # 5 秒内复用缓存（覆盖单条消息处理周期）


def get_relationship(character_id: int = 0) -> tuple:
    """获取当前好感度和信任度，返回 (affection, trust)。5秒内缓存避免重复查询。"""
    now = time.time()
    with _rel_cache_lock:
        cached = _rel_cache.get(character_id)
        if cached and cached["value"] and now - cached["ts"] < _REL_CACHE_TTL:
            return cached["value"]
    ...
    with _rel_cache_lock:
        _rel_cache[character_id] = {"ts": now, "value": result}
    return result
...
def invalidate_relationship_cache(character_id: int = 0):
    """写入后手动失效缓存，确保下次读取拿到最新值"""
    with _rel_cache_lock:
        _rel_cache.pop(character_id, None)


def get_ai_emotion(character_id: int = 0) -> str:
...
def get_emotion_behavior_hint(character_id: int = 0) -> str:
    """根据当前情感状态生成语气提示（仅影响语气，不影响行为长度）"""
    emotion = get_ai_emotion(character_id=character_id)
...
def _get_current_value(key: str, character_id: int = 0) -> float:
    """读取当前好感/信任值"""
    from core.db import get_chat_conn
    conn = get_chat_conn(character_id)
    cursor = conn.cursor()
...
def _cross_multiplier(key: str, character_id: int = 0) -> float:
...
# per-character 每日上限锁：{character_id: Lock}
_daily_cap_locks: dict = {}
_daily_cap_locks_guard = threading.Lock()


def _get_daily_cap_lock(character_id: int) -> threading.Lock:
    """获取指定角色的每日上限锁（per-character，避免跨角色阻塞）"""
    with _daily_cap_locks_guard:
        lock = _daily_cap_locks.get(character_id)
        if lock is None:
            lock = threading.Lock()
            _daily_cap_locks[character_id] = lock
        return lock


def _apply_daily_cap(key: str, delta: float, character_id: int = 0) -> float:
    """长久模式每日上限：跨天自动清零，超出上限的裁剪。
    _rel_daily_* 存在角色库 config 表（per-character 隔离，不跨角色共享）。
    """
    lock = _get_daily_cap_lock(character_id)
    with lock:
        ...
        conn = get_chat_conn(character_id)
        cursor = conn.cursor()
        # 角色库 config 表（与主库同名同结构，per-character 隔离）
...
def _update_value(key: str, delta: float, character_id: int = 0) -> float:
    """更新单个值，带渐进乘数 + 长久模式上限。返回实际应用的 delta。"""
    if abs(delta) < 0.001:
        return 0.0

    current = _get_current_value(key, character_id)
    delta *= _progressive_multiplier(current, delta)
    delta *= _cross_multiplier(key, character_id)

    mode = get_config("relationship_mode", "fast")
    if mode == "long_term":
        delta = _apply_daily_cap(key, delta * LONG_TERM_SCALE, character_id)
...
def switch_relationship_mode(new_mode: str, character_id: int = 0) -> dict:
    """切换关系模式。返回结果 dict。
    - 1天冷却
    ...
    if new_mode == "fast":
        ...
        from core.db import get_chat_conn as _gcr
        if pre_a is not None:
            _gcr(character_id).cursor().execute("UPDATE relationship SET value=? WHERE key='affection'", (float(pre_a),))
            clamped = True
            clamp_reason = f"恢复长久模式前的原值 {pre_a}"
        if pre_t is not None:
            _gcr(character_id).cursor().execute("UPDATE relationship SET value=? WHERE key='trust'", (float(pre_t),))
        _gcr(character_id).commit()
        invalidate_relationship_cache(character_id)
...
def adjust_relationship(sentiment_result: dict = None, hours_since_last: float | None = 0, character_id: int = 0, user_message: str = ""):
    ...
    Args:
        ...
        user_message: 用户原文，用于 intensity + 反讽检测（增强 LLM 判断）
    """
    ...
    # ─── intensity + 反讽增强（LLM 判断之外的信号）───
    if user_message:
        try:
            from core.emotion_patterns import detect_emotion_intensity, is_sarcastic
            emo, intensity = detect_emotion_intensity(user_message)
            # 反讽 = 用户阴阳怪气，关系受损
            if is_sarcastic(user_message):
                affection_delta -= 0.3
                a_reasons.append("用户反讽（阴阳怪气）")
            # 高强度正面 → 额外好感加成
            elif intensity > 0.7 and emo in ("happy", "love", "surprise"):
                affection_delta += 0.1
                a_reasons.append(f"高强度正面情感({emo})")
            # 高强度负面 → 额外惩罚
            elif intensity > 0.7 and emo == "sad":
                affection_delta -= 0.2
                a_reasons.append(f"高强度负面情感({emo})")
        except Exception:
            pass

    actual_a = _update_value("affection", affection_delta, character_id)
    actual_t = _update_value("trust", trust_delta, character_id)
...
def _log_emotion_event(a_delta: float = 0, t_delta: float = 0, a_reasons: list = None, t_reasons: list = None, trigger: str = "", character_id: int = 0):
    """记录一次情感调整事件到 emotion_log"""
    affection, trust = get_relationship(character_id=character_id)
    ai_emotion = get_ai_emotion(character_id=character_id)
    now = datetime.now().isoformat()
...
def get_relationship_hint(character_id: int = 0) -> str:
    ...
    # 性格特征提示（来自 emotion_evolution，per-character）
    trait_hint = ""
    try:
...
def reset_relationship(character_id: int = 0):
    """重置好感度/信任度为初始值（同时重置累积量、模式、冷却）
    per-character：relationship 和 _rel_daily_* 都在角色库，模式/冷却/预钳值在主库 config。
    """
    now = datetime.now().isoformat()
    conn = get_chat_conn(character_id)
    cursor = conn.cursor()
    ...
    # 重置角色库的每日累积量
    cursor.execute("INSERT OR REPLACE INTO config (key, value, updated_at) VALUES ('_rel_daily_affection', '0', ?)", (now,))
    cursor.execute("INSERT OR REPLACE INTO config (key, value, updated_at) VALUES ('_rel_daily_trust', '0', ?)", (now,))
    cursor.execute("INSERT OR REPLACE INTO config (key, value, updated_at) VALUES ('_rel_daily_date', '', ?)", (now,))
    conn.commit()
    invalidate_relationship_cache(character_id)
...
def set_relationship_by_foundation(foundation_type: str, character_id: int = 0):
    """根据基石类型设置初始好感和信任值"""
    from core.prompt_builder import get_foundation_defaults
...
def get_intimacy_instruction(character_id: int = 0) -> str:
    """根据关系深度生成亲密指令，注入 prompt"""
    affection, trust = get_relationship(character_id=character_id)
...
_MILESTONE_THRESHOLDS = [30, 50, 70]


def check_milestone(character_name: str = "", character_id: int = 0) -> str | None:
    """检查是否触发关系里程碑。返回提示文本或 None。"""
    affection, trust = get_relationship(character_id=character_id)
    level = (affection + trust) / 2

    for threshold in _MILESTONE_THRESHOLDS:
        flag_key = f"_milestone_{threshold}_triggered"
        already = get_config(flag_key, "")
        if not already and level >= threshold:
            set_config(flag_key, "1")
            if character_name:
                return f"【里程碑】你和「{character_name}」的关系更近了一步（好感度达到 {threshold}）"
            return f"【里程碑】关系更近了一步（好感度达到 {threshold}）"
    return None
