...
def determine_tier(msg_count, character_id: int = 0):
    """根据消息计数返回需要触发的摘要级别列表（差值比较，避免取模漂移）。"""
    tiers = []
    for level, threshold in TIER_THRESHOLDS.items():
        last = db.get_last_trigger_at(level, character_id=character_id)
...
def process_mentions(user_message, character_id: int = 0):
    ...
    for s in mentioned:
        ...
        if actual_boost > 0:
            db.increment_mention(s["id"], actual_boost, character_id=character_id)
...
# ========== 主流程 ==========
def run_summary_pipeline(msg_count, user_message="", topic_relevant=True, character_id: int = 0):
    """摘要生成主入口（per-character），根据消息计数判断触发级别并生成对应摘要。"""
    tiers = determine_tier(msg_count, character_id=character_id)
    if not tiers:
        return
    ...
    for level in tiers:
        if level == 1:
            msgs = db.get_messages_since_last_tiered_summary(limit=30, character_id=character_id)
            existing = None
        elif level == 2:
            msgs = db.get_messages_since_last_tiered_summary(limit=50, character_id=character_id)
            existing = None
        else:  # level 3
            existing = db.get_tiered_summaries_by_level(2, character_id=character_id)
            if not existing:
                logger.info(f"[摘要] Level 3 跳过：尚无 Level 2 摘要")
                db.set_last_trigger_at(level, msg_count, character_id=character_id)
                continue
        ...
        db.add_tiered_summary(
            ...
            source_ids, character_id=character_id
        )
        db.set_last_trigger_at(level, msg_count)
...
def run_decay_check(character_id: int = 0):
    """执行每日衰减检查（per-character）：重置提及增量、衰减寿命、归档过期摘要。"""
    from core.db import get_char_config, set_char_config
    last_decay = get_char_config("_last_decay_date", character_id=character_id, default="")
    ...
    db.reset_daily_boost(character_id=character_id)
    db.decay_summaries(days, character_id=character_id)
    archived = db.archive_expired_summaries(character_id=character_id)
    set_char_config("_last_decay_date", today, character_id=character_id)
    if archived > 0:
        logger.info(f"[衰减] 角色{character_id} 已归档 {archived} 条过期摘要，减少 {days} 天")
    return days


def _generate_rolling_summary(character_id: int = 0):
    ...
    try:
        ...
        # 获取最近 30 条消息（per-character）
        messages = get_recent_chat_messages(30, character_id=character_id)
        if not messages or len(messages) < 10:
            return

        # 获取已有滚动摘要（per-character 配置）
        ...
        if rolling:
            # 从结论引擎获取关键实体（per-character）
            conclusion_hint = ""
            try:
                from core.conclusion_engine import get_active_conclusions
                conclusions = get_active_conclusions(3, character_id=character_id)
                if conclusions:
...
只输出摘要文本，不要解释。"""
        ...
        if result and len(result) > 20:
            set_char_config("_rolling_summary", result.strip(), character_id=character_id)
            logger.info(f"[滚动摘要] 角色{character_id} 更新: {result[:60]}...")

    except Exception as e:
        logger.warning(f"[滚动摘要] 生成失败: {e}")


def run_background_tasks(msg_count, user_message, character_id: int = 0):
    ...
    if not _summary_lock.acquire(timeout=10):
        ...
        return
    try:
        run_decay_check(character_id=character_id)
        process_mentions(user_message, character_id=character_id)
        run_summary_pipeline(msg_count, user_message, topic_relevant, character_id=character_id)
    except Exception as e:
    ...
    if msg_count % 15 == 0 and msg_count > 0:
        if _summary_lock.acquire(timeout=5):
            try:
                _generate_rolling_summary(character_id=character_id)
            except Exception as e:
    ...
    if _summary_lock.acquire(timeout=5):
        try:
            _update_keyword_strength(_extract_keywords(user_message), character_id=character_id)
            if msg_count % 30 == 0:
                generate_reflection(character_id=character_id)
            if msg_count % 50 == 0:
                generate_deep_reflection(character_id=character_id)
        except Exception as e:
            logger.warning(f"[后台任务] 反思异常: {e}")
        finally:
            _summary_lock.release()

    # Step 4: 记忆巩固（每 100 条消息触发一次，per-character）
    if msg_count % 100 == 0 and msg_count > 0:
        try:
            from core.memory_consolidation import consolidate_memories
            result = consolidate_memories(character_id=character_id)
    ...
    if msg_count % 50 == 0 and msg_count >= 50:
        try:
            ...
            run_conclusion_analysis(character_id=character_id)
        except Exception as e:
...
_KEYWORD_STRENGTH = {}  # {character_id: {keyword: count}} — 按角色隔离
_KEYWORD_STRENGTH_LOCK = threading.Lock()
_KW_REFLECT_THRESHOLD = 10


def _load_keyword_strength():
    """从 DB 加载所有角色关键词强度（启动时调用）。
    兼容旧格式：若旧版 _keyword_strength 有数据则迁移到角色 0。"""
    ...
    try:
        ...
        if raw:
            ...
            if isinstance(old, dict):
                # 检查是旧格式 {keyword: count} 还是新格式 {cid: {keyword: count}}
                keys = list(old.keys())
                if keys and not keys[0].isdigit():
                    # 旧格式 {keyword: count} → 迁移到角色 0
                    _KEYWORD_STRENGTH["0"] = old
                    set_config("_keyword_strength_0", raw)
                    set_config("_keyword_strength", "")  # 清空旧键
        # 尝试加载各角色数据
        for cid_key in ("0", "1", "2", "3", "4", "5"):
            raw = get_config(f"_keyword_strength_{cid_key}", "")
            if raw:
                _KEYWORD_STRENGTH[cid_key] = json.loads(raw)
    except Exception as e:
        logger.warning(f"[kw] 加载关键词强度失败: {e}")


def _save_keyword_strength(character_id: int = 0):
    """持久化指定角色的关键词强度到 DB。"""
    try:
        with _KEYWORD_STRENGTH_LOCK:
            cid_str = str(character_id)
            data = dict(_KEYWORD_STRENGTH.get(cid_str, {}))
            set_config(f"_keyword_strength_{character_id}", json.dumps(data, ensure_ascii=False))
    except Exception as e:
...
def _update_keyword_strength(keywords: list, character_id: int = 0):
    """更新指定角色的关键词出现频次（线程安全），定期清理低频词。"""
    global _KW_SAVE_COUNTER
    need_save = False
    with _KEYWORD_STRENGTH_LOCK:
        cid_str = str(character_id)
        if cid_str not in _KEYWORD_STRENGTH:
            _KEYWORD_STRENGTH[cid_str] = {}
        kws = _KEYWORD_STRENGTH[cid_str]
        for kw in keywords:
            kws[kw] = kws.get(kw, 0) + 1
        # 定期清理低频关键词，防止内存膨胀
        if len(kws) > 500:
            to_remove = [k for k, v in kws.items() if v < 3]
        ...
        if _KW_SAVE_COUNTER >= 5:
            ...
            need_save = True
    if need_save:
        _save_keyword_strength(character_id)


def _check_keyword_reflection_trigger(character_id: int = 0) -> str | None:
    """检查指定角色是否有关键词累积次数达到阈值，触发专题反思。"""
    with _KEYWORD_STRENGTH_LOCK:
        cid_str = str(character_id)
        kws = _KEYWORD_STRENGTH.get(cid_str, {})
        for kw, count in list(kws.items()):
            if count >= _KW_REFLECT_THRESHOLD:
                kws[kw] = 0  # 重置
                try:
                    set_config(f"_keyword_strength_{character_id}", json.dumps(kws, ensure_ascii=False))
                except Exception:
                    pass
                return kw
    return None
...
def generate_deep_reflection(character_id: int = 0):
    """分层反思（per-character）：从已有反思和关键词触发中提取更高阶的用户洞察。"""
    try:
        from core.db import get_recent_chat_messages
        from core.llm_client import call_llm
        ...
        # 收集已有反思和近期对话
        with _get_chat(character_id) as conn:
            cursor = conn.cursor()
            cursor.execute(
        ...
        # 检查是否有关键词触发
        trigger_kw = _check_keyword_reflection_trigger(character_id=character_id)
        if not trigger_kw:
            return  # 没有触发
...
def generate_reflection(character_id: int = 0):
    """从近期对话中生成反思摘要（关注/情绪/潜在需求），存入 Level 2 摘要（per-character）。"""
    try:
        from core.db import get_recent_chat_messages
        from core.llm_client import call_llm

        # 读取最近 50 条用户消息
        messages = get_recent_chat_messages(50, character_id=character_id)
        ...
        # 更新触发计数器，防止重复生成
        mc = db.get_msg_counter(character_id=character_id)
        db.set_last_trigger_at(2, mc, character_id=character_id)
