...
def add_document_vectors(doc_id, chunks, filename="", character_id: int = 0):
    """将文档分块嵌入并存入 knowledge_base 集合（per-character via metadata）"""
    ...
    for i, chunk in enumerate(chunks):
        ...
        try:
            ...
            collection.upsert(
                ...
                metadatas=[{"doc_id": str(doc_id), "chunk_index": i, "filename": filename, "character_id": character_id}],
                documents=[chunk]
            )
            count += 1
