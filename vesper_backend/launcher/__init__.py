...
"""夕语启动器包主入口——负责单实例检测、窗口创建、后端启动、托盘及天气通知"""
...
<div class="box">
<div class="logo-wrap"><h1 class="logo">夕　语</h1></div>
...
def run():
    ...
    logger.info(f"[启动] 使用端口: {VESPER_PORT}")
    _html = loading_html.replace('__PORT__', _json.dumps(VESPER_PORT)).replace('__THEME__', _json.dumps(_last_theme))
    ...
    if os.path.exists(_prefs_path):
        try:
            ...
            _geo = (_prefs.setdefault("profile", {})
                   ...
                   .setdefault("exceptions", {})
                   .setdefault("geolocation", {}))
            _key = f"http://127.0.0.1:{VESPER_PORT},*"
            if _key not in _geo:
                _geo[_key] = {"setting": 1, "last_modified": str(int(_time.time() * 1000000))}
                with open(_prefs_path, "w", encoding="utf-8") as _f:
                    _json.dump(_prefs, _f, ensure_ascii=False)
                logger.info(f"[启动] 已预授权地理位置权限（端口 {VESPER_PORT}）")
        except Exception as e:
    ...
    try:
        _s._window = webview.create_window(
            title="夕语", html=_html, width=ww, height=wh,
    ...
    logger.info(f"[启动] 命名管道服务器已启动 (vesper_ai_{VESPER_PORT})")
