...
"""launcher 窗口管理——主题读写、图标设置、HWND 管理、WndProc 消息处理"""
import os
import sys
import ctypes
...
def _read_theme():
    ...
    db_path = os.path.join(_s.BASE_DIR, "data", "vesper.db")
...
def _show_toast(title, body, duration="short"):
    ...
    if _s._has_winotify:
        try:
            from winotify import Notification
            Notification(app_id="夕语", title=title, msg=body, duration=duration).show()
            return
        except Exception as e:
