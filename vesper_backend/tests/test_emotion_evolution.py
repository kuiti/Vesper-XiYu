...
class TestUpdateTrait:
    ...
    @patch("core.emotion_evolution.get_chat_conn")
...
class TestGetTraitProfile:
    @patch("core.emotion_evolution.get_config", return_value="夕语")
...
class TestRuleSilenceDecay:
    ...
    def test_3_days_triggers_decay(self, mock_last, mock_rel, mock_trait, mock_log, mock_set):
        """3 天以上沉默应触发衰减（per-character，默认 0）。"""
        _rule_silence_decay(72)  # 3天
        # affection -0.2 * 1, trust -0.1 * 1 (first run, days_since_last=1)
        mock_rel.assert_any_call("affection", -0.2, 0)
        mock_rel.assert_any_call("trust", -0.1, 0)
    ...
    def test_7_plus_days_reduces_initiative(self, mock_last, mock_rel, mock_trait, mock_log, mock_set):
        ...
        mock_trait.assert_called_with("conscientiousness", -0.05, 0)
...
class TestProcessDailyEvolution:
    ...
    def test_runs_all_rules(self, mock_dt, mock_conn, mock_claim, mock_hours,
        ...
        mock_conn.return_value.__exit__ = MagicMock(return_value=False)
        process_daily_evolution()
        # 规则按角色遍历调用（_enumerate_character_ids 返回 [0, 1]）
        mock_silence.assert_any_call(10, 0)
        mock_silence.assert_any_call(10, 1)
        mock_cold.assert_any_call(0)
        mock_cold.assert_any_call(1)
        mock_habit.assert_any_call(0)
        mock_habit.assert_any_call(1)
        # _rule_interaction_quality 现在按角色遍历调用（character_id 参数）
        # 至少调过 1 次，且所有参数都是 int
        assert mock_quality.called
        for call in mock_quality.call_args_list:
            args, kwargs = call
            # 要么位置参数第一个是 int，要么 kwargs 里有 character_id=int
            if args:
                assert isinstance(args[0], int)
            elif 'character_id' in kwargs:
                assert isinstance(kwargs['character_id'], int)
