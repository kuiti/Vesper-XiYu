...
class TestDSMLFiltering:
    """DSML 工具调用标记 — 已移除过滤逻辑，DSML 内容现在原样保留。"""

    def test_dsml_not_filtered_in_thinking_state(self):
        p = ThinkingParser()
        dsml = '<｜｜DSML｜｜tool_calls>invoke stuff</｜｜DSML｜｜tool_calls>'
        p.feed("思考" + dsml)
        # DSML 不再被过滤，保留在 buffer 中
        assert "DSML" in p.buffer

    def test_dsml_not_filtered_in_reply_state(self):
        p = ThinkingParser()
        p.feed("思考【回复】")
        dsml = '<｜｜DSML｜｜tool_calls>invoke stuff</｜｜DSML｜｜tool_calls>'
        result = p.feed(dsml)
...
class TestExtractReplyFromBuffer:
    ...
    def test_dsml_not_removed_from_buffer(self):
        """DSML 不再从 buffer 中移除。"""
        p = ThinkingParser()
