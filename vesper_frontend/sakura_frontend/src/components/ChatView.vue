<template>
  <div class="chat-view">
    ...
    <div class="chat-top-bar">
      ...
      <span class="ctb-sep" v-if="totalMessages && ipCity">·</span>
      ...
      <div v-for="(msg, idx) in messages" :key="msg.id || idx">
        ...
        <div :class="['msg', msg.role, { proactive: msg.isProactive, surprise: msg.isSurprise }]">
          ...
          <div class="msg-body">
            ...
            <span v-if="msg.role === 'assistant' && msg._emotion" class="emotion-icon" :title="emotionLabel(msg._emotion)">{{ emotionIcon(msg._emotion) }}</span>
            ...
            <div class="msg-footer">
              ...
              <button v-if="msg.role === 'assistant' && msg.content && (voiceSettings?.tts_enabled || characterVoice)" class="tts-btn" @click="speakText(msg)" :title="ttsPlaying && ttsMsgId === msg.id ? '停止' : '朗读'">
...
export default {
  ...
  props: {
    ...
    ipCity: String, ipCityShort: String, totalMessages: Number, chatFontSize: Number, kaomojis: Array, historyReady: Boolean,
  ...
  methods: {
    ...
    async _doSpeak(msg) {
      ...
      try {
        let res
        if (this.characterVoice && Object.keys(this.characterVoice).length > 0) {
          // 角色卡语音配置
          res = await api.post('/tts/speak', {
            text: msg.content,
            voice_config: this.characterVoice,
          })
        } else {
          // 默认 TTS
          const mode = this.voiceSettings?.tts_clone_mode || 'preset'
          res = await api.post('/tts/tts', {
            text: msg.content,
    ...
    emotionIcon(emotion) {
      const icons = { happy: '😊', sad: '😢', angry: '😠', surprise: '😲', calm: '😌', love: '🥰' }
      return icons[emotion] || '😌'
    },
    emotionLabel(emotion) {
      const labels = { happy: '开心', sad: '难过', angry: '生气', surprise: '惊讶', calm: '平静', love: '喜欢' }
...
.emotion-icon { font-size: 16px; margin-left: 4px; vertical-align: middle; }
